...
class HttpService(IHttpService):
    ...
    URLs are built by combining the base URL with relative URLs.

    Parameters
    ----------
    base_url : str
        The base URL to be used with every request. Should be passed **without**
        a trailing forward slash. Example: ``https://example.com``.
    default_headers : dict, optional
        Default headers to include with every request. Example: 
        ``{"User-Agent": "Example"}``.
    timeout : float, optional
        Timeout in seconds for blocking operations (e.g., connecting to the server,
        reading data). Defaults to 1.0.
    """
    def __init__(
        self,
        base_url: str,
    ...
    def _build_headers(self, additional_headers: dict | None) -> dict:
        ...
        if not additional_headers and self.default_headers:
    ...
    def _build_parameters(self, abs_url, parameters):
    ...
    def _is_valid_relative_url(self, rel_url):
    ...
    def get(self, relative_url: str = None, headers: dict[str, str] | None = None, params: dict[str, str] | None = None) -> HttpResponse:
        ...
        request = Request(request_url, headers, method="GET")
        response = None
        try:
